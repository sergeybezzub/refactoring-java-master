...
- ✔️ Added runtime exception `MovieRentalDuplicationException.java`. It will be thrown by `MovieRentalManager` if someone will try to rent a movie that has already rented.
...
- 🔹 Refactored `RentalInfo.java`. The statement() method was significantly simplified and become auto extended using `MovieRentalData.java`.

#### 🛠️ Fixed
- 🐛 Fixed `Movie.java`. Variable `String code;` has been refactored to `MovieRentalData code;`. Constant variable has a lot of advantages if compare it with String variable. For example, for enumeration we make sure we have only correct values for that variable. We can't guaranty that if we use a String variable. 
...
#### ❌ Removed
- 🗑️ Class `Main.java`. That class contains only simple, hardcoded, useless test. The standard Unit tests were added instead. 
...
## Next possible steps
- 🔹Create SpringBoot configuration for application to have all set of advantages to extend and support that application in the future 
- 🔹Add support of database using Spring Data and create some implementation for `MovieStorage` interface
- 🔹Add support `REST API` using Spring Web to have a remote API for that application
- 🔹Add security support using Spring Security
